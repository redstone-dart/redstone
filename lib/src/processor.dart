...
class Processor implements Manager {
  ...
  void addRouteWrapper(Type metadataType, RouteWrapper wrapper, 
    ...
    _findHandlers(_getRoutes, metadataType, includeGroups)
        .forEach((m) => m.handler.wrappers.add(
            new RouteWrapperMetadata(wrapper, m.metadata)));
  }
  ...
  void addResponseProcessor(Type metadataType, ResponseProcessor processor, 
                            {bool includeGroups: false}) {
    _findHandlers(_getRoutes, metadataType, includeGroups)
         .forEach((m) => m.handler.responseProcessors.add(
             new ResponseProcessorMetadata(processor, m.metadata)));
  }
  ...
  List<HandlerMetadata> _getRoutes(dynamic metadata) {
    if (metadata is ApplicationMetadata) {
      ...
      return []..addAll(metadata.defaultRoutes)
  ...
  List<_Match> _findHandlers(List<HandlerMetadata> getHandlers(metadata),
    ...
    var result = [];
    getHandlers(serverMetadata).forEach((h) =>
  ...
  List<_Match> _findHandlersByParams(List<HandlerMetadata> getHandlers(metadata),
                                     Type metadataType) {
      
    var result = [];
    getHandlers(serverMetadata).forEach((h) =>
      (h.mirror as MethodMirror).parameters.forEach((p) {
  ...
  void _initializeInjector() {
    ...
    serverMetadata.groups.forEach((g) 
        => module.bind(g.mirror.reflectedType));
    modules.add(module);
  ...
  Iterable<MethodMirror> _findDeclaredFunctions() =>
    serverMetadata.loadedLibraries.expand((LibraryMirror ldef) =>
      ldef.declarations.values).where((d) => d is MethodMirror);

  Iterable<ClassMirror> _findDeclaredClasses() =>
    serverMetadata.loadedLibraries.expand((LibraryMirror ldef) =>
  ...
  InterceptorInvoker _wrapInterceptor(InterceptorMetadata interceptor, 
    ...
    return (RequestParser request) async {
      ...
      return await owner.invoke(interceptor.mirror.simpleName, 
  ...
  ErrorHandlerInvoker _wrapErrorHandler(ErrorHandlerMetadata errorHandler,
    ...
    return (Request request) async {
      ...
      return await owner.invoke(errorHandler.mirror.simpleName, 
          positionalArgs, namedArgs).reflectee;
  ...
  InterceptorInvoker _wrapDynamicInterceptor(Interceptor conf, 
                                             DynamicHandler interceptor) {
    return (RequestParser request) async {
  ...
  ErrorHandlerInvoker _wrapDynamicErrorHandler(ErrorHandler conf,
                                               String handlerName, 
                                               DynamicHandler errorHandler) {
    return (Request request) async {
