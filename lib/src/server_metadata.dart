...
/// returned by routes.
typedef dynamic ResponseProcessor(dynamic metadata, String handlerName,
...
class LibraryMetadata extends HandlerMetadata<Install, LibraryMirror>
  ...
  LibraryMetadata(Install conf, LibraryMirror mirror,
                  List metadata, this.dependencies) : 
...
class ApplicationMetadata {
  ...
  ApplicationMetadata(this.routes, this.interceptors,
                      this.errorHandlers, this.groups);
...
abstract class RequestTargetMetadata implements 
    HandlerMetadata<RequestTarget, MethodMirror> { 
...
class RouteMetadata extends HandlerMetadata<Route, MethodMirror>
  ...
  RouteMetadata(this.library, Route conf, MethodMirror mirror,
                List metadata, [String name]) :
...
class DefaultRouteMetadata extends HandlerMetadata<DefaultRoute, MethodMirror> 
  ...
  DefaultRouteMetadata(this.library, DefaultRoute conf, MethodMirror mirror,
                List metadata, [String name]) :
      super(conf, mirror, metadata, name);
...
class InterceptorMetadata extends
  ...
  InterceptorMetadata(this.library, Interceptor conf, MethodMirror mirror,
                      List metadata, [String name]) :
      super(conf, mirror, metadata, name);
...
class ErrorHandlerMetadata extends
  ...
  ErrorHandlerMetadata(this.library, ErrorHandler conf, MethodMirror mirror,
                       List metadata, [String name]) :
      super(conf, mirror, metadata, name);
...
class GroupMetadata extends HandlerMetadata<Group, ClassMirror> {
  ...
  GroupMetadata(this.library, 
                Group conf, ClassMirror mirror, List metadata,
                this.defaultRoutes,
                this.routes,
                this.interceptors,
                this.errorHandlers) :
