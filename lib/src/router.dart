...
class Router {
  ...
  void _buildShelfHandler() {
    ...
    var forwardPipeline = new shelf.Pipeline()
        .addMiddleware(_forwardMiddleware);
    _serverCtx.shelfContext.middlewares.forEach(
        (m) => pipeline = pipeline.addMiddleware(m));
  ...
  void _loadHandlers([String pathPrefix, List<int> chainIdxByLevel, 
                      LibraryMetadata lib]) {
    
    if (lib == null) {
      _serverCtx.serverMetadata.rootLibraries.forEach(
          (l) => _loadHandlers(null, [], l));
  ...
  void _loadDynamicHandlers() {
    ...
    _serverCtx.serverMetadata.routes
        .where((r) => r.library == null)
        .forEach((r) => 
            _targetListBuilder.add(
                new UrlTemplate(r.conf.urlTemplate), r.conf.methods, 
                  _serverCtx.routeInvokers[r]));
    
    _interceptorListBuilder.add(
        _serverCtx.serverMetadata.interceptors
            .where((i) => i.library == null)
            .map((i) => 
                new _Interceptor(new RegExp(i.conf.urlPattern), 
    ...
    _serverCtx.serverMetadata.errorHandlers
        ...
        .forEach((e) =>
            _errorHandlerMapBuilder.add(e.conf.statusCode, 
                new _ErrorHandler(
                  e.conf.urlPattern != null ? new RegExp(e.conf.urlPattern) : null, 
                  _serverCtx.errorHandlerInvokers[e])));
    
  }
  
  void _getTargets(String pathPrefix, 
                   LibraryMetadata lib) {
    ...
    for (GroupMetadata group in lib.groups) {
      ...
      for (RouteMetadata route in group.routes) {
        ...
        var url = null;
        if (pathPrefix != null) {
          url = _joinUrl(_joinUrl(pathPrefix, group.conf.urlPrefix), 
              route.conf.urlTemplate);
          urlTemplate = new UrlTemplate(url);
        } else {
  ...
  void _getInterceptors(String pathPrefix,
  ...
  void _getErrorHandlers(String pathPrefix, 
                         LibraryMetadata lib) {
...
class _ChainImpl implements Chain {
  ...
  @override
  Future<shelf.Response> forward(String url) async {
    var req = currentContext.request;
    var newUrl = req.url.resolve(url);
    var shelfReqCtx = {};
    req.attributes.forEach((k, v) {
      shelfReqCtx[k.toString()] = v;
    });
    var newReq = req.shelfRequest.change(url: newUrl, context: shelfReqCtx);
...
class _TargetListBuilder {
  ...
  void add(UrlTemplate url, List<String> methods, RouteInvoker invoker) {
    var key = url.toString();
    var target = mapTargets[key];
    if (target == null) {
      target = new _Target(url);
...
class _ErrorHandlerMapBuilder {
  ...
  Map<int, List<_ErrorHandler>> build() {
    ...
    errorHandlers.forEach((status, errorHandlerList) {
      map[status] = errorHandlerList..sort((e1, e2) {
        if (e1.urlRegex == null) {
