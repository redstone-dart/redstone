...
class ParametersProcessor implements Function {
  ...
  final List<ArgHandler> _handlers = [];
  ...
  ParametersProcessor(this.handlerName, this.parameters, 
                      this.injector, this.providers,
  ...
  Future call(Request request, 
              List positionalArgs, Map namedArgs) {
    return Future.forEach(_paramProcessors, (p) => 
        p(request, positionalArgs, namedArgs));
  }
    ...
    parameters.forEach((ParameterMirror param) {
      ...
      if (!param.metadata.isEmpty) {
          ...
          if (metadataProcessor != null) {
            ...
            argHandler = metadataProcessor(handlerName, injector, 
                  metadata, param);
      ...
      if (argHandler == null) {
        if (_defaultArgHandler != null) {
          ...
          argHandler = _defaultParamHandler(handlerName, injector, 
              null, param);
        }
      }
      
      var processor = (Request request, 
                       List positionalArgs, Map namedArgs) async {
  ...
  ArgHandler call(String handlerName, Injector injector,
    ...
    try {
      ...
      throw new SetupException(handlerName, 
          "Invalid parameter: Can't inject $name");
    }
...
dynamic convertValue(Converter converter, String paramName, 
                     String value, String handlerName) {
  try {
    return converter(value);
  } catch(e) {
    throw new RequestException(handlerName, 
