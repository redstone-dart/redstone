...
class Scanner {
  ...
  LibraryMetadata _scanLibrary(LibraryMirror mirror, [Install conf]) {
    ...
    if (dependencies.isEmpty && lib.routes.isEmpty && 
        lib.interceptors.isEmpty && lib.errorHandlers.isEmpty && 
        lib.groups.isEmpty) {
      return null;
    }
  ...
  RouteMetadata _loadRoute(LibraryMetadata lib, 
                           MethodMirror mirror, Route conf) {
    var metadata = mirror.metadata.map((m) => m.reflectee)
        .toList(growable: false);
    
    return new RouteMetadata(lib, conf, mirror, metadata);
  }
  
  InterceptorMetadata _loadInterceptor(LibraryMetadata lib, 
                                       MethodMirror mirror, 
                                       Interceptor conf) {
    var metadata = mirror.metadata.map((m) => m.reflectee)
         .toList(growable: false);
  ...
  ErrorHandlerMetadata _loadErrorHandlers(LibraryMetadata lib, 
                                          MethodMirror mirror, 
                                          ErrorHandler conf) {
    var metadata = mirror.metadata.map((m) => m.reflectee)
         .toList(growable: false);
  ...
  GroupMetadata _loadGroup(LibraryMetadata lib, 
                           ClassMirror mirror, 
                           Group conf) {
    var metadata = mirror.metadata.map((m) => m.reflectee)
          .toList(growable: false);
    ...
    return new GroupMetadata(lib, conf, mirror, metadata, 
        defaultRoutes, routes, interceptors, errorHandlers);
    
  }
  
  DefaultRouteMetadata _loadDefaultRoutes(LibraryMetadata lib, 
                                          MethodMirror mirror, 
                                          DefaultRoute conf) {
    var metadata = mirror.metadata.map((m) => m.reflectee)
         .toList(growable: false);
